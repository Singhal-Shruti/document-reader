...
from agents_app.commands._shared import (
    add_chroma_args,
    add_chunk_args,
    run_ingestion_pipeline,
)
from agents_app.loaders import load_swagger
...
def add_arguments(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        COMMAND,
        help=(
            "Load a Swagger/OpenAPI JSON or YAML spec via the OpenAPI Toolkit "
            "and store endpoint chunks in ChromaDB."
        ),
    )
    parser.add_argument(
        "source",
    ...
    parser.add_argument(
        ...
        dest="dereference",
        action="store_false",
        help="Skip $ref dereferencing (faster, but less context per endpoint).",
    )
    parser.add_argument(
        "--no-overview",
...
def run(args: argparse.Namespace) -> None:
    ...
    run_ingestion_pipeline(args, documents, source_label=f"Swagger ({args.source})")
